 ...
 */
class TTS(val IVA: Context,val mHandler:Handler) {
    ...
    protected var offlineVoice = OfflineResource.VOICE_DUYY
    ...
    protected val params: HashMap<String, String>
        get() {
            ...
            eLog("设置在线发声音人:" + IVA.eGetDefaultSharedPreferences("set_tts_load_model"))
//设置本地发音人
            params[SpeechSynthesizer.PARAM_SPEAKER] = when (IVA.eGetDefaultSharedPreferences("set_tts_load_model")) {
                "F" -> "0"
                "M" -> "1"
                "X" -> "3"
                "Y" -> "4"
...
//                    "4"
            ...
            //            设置合成的音量，0-9 ，默认 9
            params[SpeechSynthesizer.PARAM_VOLUME] = IVA.eGetDefaultSharedPreferences("set_PARAM_VOLUME").toString() ?: "9"
            // 设置合成的语速，0-9 ，默认 5
            params[SpeechSynthesizer.PARAM_SPEED] = IVA!!.eGetDefaultSharedPreferences("set_PARAM_SPEED").toString() ?: "5"
            // 设置合成的语调，0-9 ，默认 5
            params[SpeechSynthesizer.PARAM_PITCH] = IVA!!.eGetDefaultSharedPreferences("set_PARAM_PITCH").toString() ?: "5"
     ...
     */
    public fun loadModel(mode: String) {
        offlineVoice = mode
        val offlineResource = createOfflineResource(offlineVoice)
...
//        checkResult(result, "loadModel")
    ...
    init {
        initialTts()
//       var mode=IVA.eGetDefaultSharedPreferences("set_tts_load_model").toString()?:"Y"
        var mode=IVA.eGetDefaultSharedPreferences("set_tts_load_model").toString()?:"F"
        eLog("mode:"+mode)
//        if(mode.equals("")) mode="Y"
        if(mode.equals("")) mode="F"
        loadModel(mode)
    ...
    protected fun initialTts() {
        ...
        synthesizer = NonBlockSyntherizer(IVA, initConfig, mHandler!!) // 此处可以改为MySyntherizer 了解调用过程
    }
    ...
    protected fun createOfflineResource(voiceType: String): OfflineResource? {
        ...
        try {
            offlineResource = OfflineResource(IVA, voiceType)
        } catch (e: IOException) {
            // IO 错误自行处理
            e.printStackTrace()
