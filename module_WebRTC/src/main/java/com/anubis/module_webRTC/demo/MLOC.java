    ...
    public static void showDialog(final Context context, final JSONObject data){
        try {
            ...
            if(dialogs[0]==null||dialogs[0].isShowing()==false){
                dialogs[0] = new Dialog(context, com.anubis.module_webRTC.R.style.dialog_notify);
            ...
            ((TextView) dialogs[0].findViewById(com.anubis.module_webRTC.R.id.msg_info)).setText(msg);
            dialogs[0].findViewById(com.anubis.module_webRTC.R.id.yes_btn).setOnClickListener(new View.OnClickListener() {
                @Override
...
//                    }
    ...
    public static int getHeadImage(Context context,String userID){
        if(mHeadIconIds==null){
            TypedArray ar = context.getApplicationContext().getResources().obtainTypedArray(com.anubis.module_webRTC.R.array.head_images);
            int len = ar.length();
