...
object eFtpClient {
     ...
     * @return
     */
data class dataIpInfo(var ip:String,var port:Int,var state:Boolean)
    fun scanService(mAPP: Application, Urls: ArrayList<String>?, FTPPort: Int = 3333, UserName: String = "sxk", UserPassword: String = "sxk", mHandler: Handler?): ArrayList<dataIpInfo?>? {
        val msg = Message()
        msg.what = FTP_SCAN
        eLog("FTP扫描")
        var infos: ArrayList<dataIpInfo?>? = arrayListOf()
        val segment =  getIP()?.split(".")
        if (Urls != null) {
            eLog("FTP定义扫描")
            for (url in Urls) {
                if (initFTP(url, FTPPort, UserName, UserPassword)) {
                    infos!!.add(dataIpInfo(url, FTPPort,true))
                    ftpClient!!.disconnect()
                    eLog("$url 成功")
                }else{
                    eLog("$url 失败")
                    infos!!.add(dataIpInfo(url, FTPPort,false))
                }
            }
        }
    ...
    var LOCAL_CHARSET = "GBK"
    fun uploadFile(LocalFilePath: String, FTPFilePath: String, FTPFileName: String = "test.jpg"): Boolean {
        if (!ftpClient!!.isConnected) {
            if (!initFTP(FTPUrl, FTPPort, UserName, UserPassword)) {
                eLog("连接失败")
                return false
        ...
        try {
            ...
            ftpClient!!.storeFile(String(FTPFileName.toByteArray(),
                    Charsets.ISO_8859_1), fileInputStream)
            //关闭文件流
