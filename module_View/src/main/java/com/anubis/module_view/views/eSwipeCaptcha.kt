 ...
 * 仿斗鱼滑动验证码View
 */
class eSwipeCaptcha @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : AppCompatImageView(context, attrs, defStyleAttr) {
    private val TAG = eSwipeCaptcha::class.java.name
    ...
    private fun init(context: Context, attrs: AttributeSet?, defStyleAttr: Int) {
        ...
        val ta = context.theme.obtainStyledAttributes(attrs, R.styleable.eSwipeCaptcha, defStyleAttr, 0)
        val n = ta.indexCount
        for (i in 0 until n) {
            val attr = ta.getIndex(i)
            if (attr == R.styleable.eSwipeCaptcha_captchaHeight) {
                mCaptchaHeight = ta.getDimension(attr, defaultSize.toFloat()).toInt()
            } else if (attr == R.styleable.eSwipeCaptcha_captchaWidth) {
                mCaptchaWidth = ta.getDimension(attr, defaultSize.toFloat()).toInt()
            } else if (attr == R.styleable.eSwipeCaptcha_matchDeviation) {
    ...
    private fun createMatchAnim() {
        ...
        mFailAnim?.addListener(object : AnimatorListenerAdapter() {
            override fun onAnimationEnd(animation: Animator) {
                onCaptchaMatchCallback!!.matchFailed(this@eSwipeCaptcha)
            }
        })
        ...
        mSuccessAnim?.addListener(object : AnimatorListenerAdapter() {
            ...
            override fun onAnimationEnd(animation: Animator) {
                onCaptchaMatchCallback!!.matchSuccess(this@eSwipeCaptcha)
     ...
     * @param value
     */
    fun eSetCurrentSwipeValue(value: Int) {
        mDragerOffset = value
        invalidate()
    }

    interface OnCaptchaMatchCallback {
        fun matchSuccess(rxSwipeCaptcha: eSwipeCaptcha)
        fun matchFailed(rxSwipeCaptcha: eSwipeCaptcha)
    }
     ...
     * @return
     */
    fun setOnCaptchaMatchCallback(onCaptchaMatchCallback: OnCaptchaMatchCallback?): eSwipeCaptcha {
