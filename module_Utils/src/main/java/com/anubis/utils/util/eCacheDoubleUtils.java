 ...
 * </pre>
 */
public final class eCacheDoubleUtils implements CacheConstants {

    private static final SimpleArrayMap<String, eCacheDoubleUtils> CACHE_MAP = new SimpleArrayMap<>();
    private eCacheMemoryUtils mCacheMemoryUtils;
    private eCacheDiskUtils mCacheDiskUtils;

    /**
     * Return the single {@link eCacheDoubleUtils} instance.
     *
     * @return the single {@link eCacheDoubleUtils} instance
     */
    public static eCacheDoubleUtils getInstance() {
        return getInstance(eCacheMemoryUtils.getInstance(), eCacheDiskUtils.getInstance());
    }

    /**
     * Return the single {@link eCacheDoubleUtils} instance.
     *
     * @param cacheMemoryUtils The instance of {@link eCacheMemoryUtils}.
     * @param cacheDiskUtils   The instance of {@link eCacheDiskUtils}.
     * @return the single {@link eCacheDoubleUtils} instance
     */
    public static eCacheDoubleUtils getInstance(@NonNull final eCacheMemoryUtils cacheMemoryUtils,
                                                @NonNull final eCacheDiskUtils cacheDiskUtils) {
        final String cacheKey = cacheDiskUtils.toString() + "_" + cacheMemoryUtils.toString();
        eCacheDoubleUtils cache = CACHE_MAP.get(cacheKey);
        if (cache == null) {
            cache = new eCacheDoubleUtils(cacheMemoryUtils, cacheDiskUtils);
            CACHE_MAP.put(cacheKey, cache);
        }
        return cache;
    }

    private eCacheDoubleUtils(eCacheMemoryUtils cacheMemoryUtils, eCacheDiskUtils cacheUtils) {
