 ...
 * </pre>
 */
public final class eCacheMemoryUtils implements CacheConstants {

    private static final int DEFAULT_MAX_COUNT = 256;

    private static final SimpleArrayMap<String, eCacheMemoryUtils> CACHE_MAP = new SimpleArrayMap<>();

    private final String                       mCacheKey;
    private final LruCache<String, CacheValue> mMemoryCache;

    /**
     * Return the single {@link eCacheMemoryUtils} instance.
     *
     * @return the single {@link eCacheMemoryUtils} instance
     */
    public static eCacheMemoryUtils getInstance() {
     ...
     * @return the single {@link eCacheMemoryUtils} instance
     */
    public static eCacheMemoryUtils getInstance(final int maxCount) {
     ...
     * @return the single {@link eCacheMemoryUtils} instance
     */
    public static eCacheMemoryUtils getInstance(final String cacheKey, final int maxCount) {
        eCacheMemoryUtils cache = CACHE_MAP.get(cacheKey);
        if (cache == null) {
            cache = new eCacheMemoryUtils(cacheKey, new LruCache<String, CacheValue>(maxCount));
            CACHE_MAP.put(cacheKey, cache);
        }
        return cache;
    }

    private eCacheMemoryUtils(String cacheKey, LruCache<String, CacheValue> memoryCache) {
        mCacheKey = cacheKey;
