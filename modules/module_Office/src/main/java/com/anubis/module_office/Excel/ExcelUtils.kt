...
internal object ExcelUtils {
     ...
     * @param colName
     */
    fun initExcel(fileName: String, colName:Array< Array<String>>, sheetNames: Array<String> ) {
        format()
        ...
        try {
            if (eIFile.eCheckFile(fileName)) {
                ...
                sheetNames.forEachIndexed { index, c ->
                    ...
                    for (col in colName[index].indices) {
                        sheet.addCell(Label(col, 0, colName[index][col], arial10format))
                    }
    ...
    fun <T> writeObjListToExcel(objList: List<T>, fileName: String, sheetIndex: Int = 0) {
