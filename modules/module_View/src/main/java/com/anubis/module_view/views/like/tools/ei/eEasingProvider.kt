 ...
 */
internal object eEasingProvider {
    /**
     * @param rxEase          Easing type
     * @param elapsedTimeRate Elapsed time / Total time
     * @return easedValue
     */
    operator fun get(rxEase: eEase, elapsedTimeRate: Float): Float {
        var elapsedTimeRate = elapsedTimeRate
        return when (rxEase) {
            eEase.LINEAR -> elapsedTimeRate
            eEase.QUAD_IN -> getPowIn(elapsedTimeRate, 2.0)
            eEase.QUAD_OUT -> getPowOut(elapsedTimeRate, 2.0)
            eEase.QUAD_IN_OUT -> getPowInOut(elapsedTimeRate, 2.0)
            eEase.CUBIC_IN -> getPowIn(elapsedTimeRate, 3.0)
            eEase.CUBIC_OUT -> getPowOut(elapsedTimeRate, 3.0)
            eEase.CUBIC_IN_OUT -> getPowInOut(elapsedTimeRate, 3.0)
            eEase.QUART_IN -> getPowIn(elapsedTimeRate, 4.0)
            eEase.QUART_OUT -> getPowOut(elapsedTimeRate, 4.0)
            eEase.QUART_IN_OUT -> getPowInOut(elapsedTimeRate, 4.0)
            eEase.QUINT_IN -> getPowIn(elapsedTimeRate, 5.0)
            eEase.QUINT_OUT -> getPowOut(elapsedTimeRate, 5.0)
            eEase.QUINT_IN_OUT -> getPowInOut(elapsedTimeRate, 5.0)
            eEase.SINE_IN -> (1f - Math.cos(elapsedTimeRate * Math.PI / 2f)).toFloat()
            eEase.SINE_OUT -> Math.sin(elapsedTimeRate * Math.PI / 2f).toFloat()
            eEase.SINE_IN_OUT -> (-0.5f * (Math.cos(Math.PI * elapsedTimeRate) - 1f)).toFloat()
            eEase.BACK_IN -> (elapsedTimeRate * elapsedTimeRate * ((1.7 + 1f) * elapsedTimeRate - 1.7)).toFloat()
            eEase.BACK_OUT -> (--elapsedTimeRate * elapsedTimeRate * ((1.7 + 1f) * elapsedTimeRate + 1.7) + 1f).toFloat()
            eEase.BACK_IN_OUT -> getBackInOut(elapsedTimeRate, 1.7f)
            eEase.CIRC_IN -> (-(Math.sqrt(1f - elapsedTimeRate * elapsedTimeRate.toDouble()) - 1)).toFloat()
            eEase.CIRC_OUT -> Math.sqrt(1f - --elapsedTimeRate * elapsedTimeRate.toDouble()).toFloat()
            eEase.CIRC_IN_OUT -> {
            ...
            eEase.BOUNCE_IN -> getBounceIn(elapsedTimeRate)
            eEase.BOUNCE_OUT -> getBounceOut(elapsedTimeRate)
            eEase.BOUNCE_IN_OUT -> {
                if (elapsedTimeRate < 0.5f) {
                    getBounceIn(elapsedTimeRate * 2f) * 0.5f
                } else getBounceOut(elapsedTimeRate * 2f - 1f) * 0.5f + 0.5f
            }
            eEase.ELASTIC_IN -> getElasticIn(elapsedTimeRate, 1.0, 0.3)
            eEase.ELASTIC_OUT -> getElasticOut(elapsedTimeRate, 1.0, 0.3)
            eEase.ELASTIC_IN_OUT -> getElasticInOut(elapsedTimeRate, 1.0, 0.45)
