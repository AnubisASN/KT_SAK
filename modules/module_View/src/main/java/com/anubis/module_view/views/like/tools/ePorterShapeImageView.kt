 ...
 * @author tamsiree
 */
open class ePorterShapeImageView : ePorterImageView {
    ...
    private fun setup(context: Context, attrs: AttributeSet?, defStyle: Int) {
        if (attrs != null) {
            val typedArray = context.obtainStyledAttributes(attrs, R.styleable.ePorterImageView, defStyle, 0)
            shape = typedArray.getDrawable(R.styleable.ePorterImageView_siShape)
            typedArray.recycle()
        }
        matrix0 = Matrix()
    }
