package px_picker.ui.camera.util;
...
public final class CameraUtils {
    ...
    public static px_picker.ui.camera.util.Size getPictureSize(List<px_picker.ui.camera.util.Size> choices, @CameraConfig.MediaQuality int mediaQuality) {
        if (choices == null || choices.isEmpty()) return null;
        if (choices.size() == 1) return choices.get(0);

        px_picker.ui.camera.util.Size result = null;
        px_picker.ui.camera.util.Size maxPictureSize = Collections.max(choices, new CompareSizesByArea2());
        px_picker.ui.camera.util.Size minPictureSize = Collections.min(choices, new CompareSizesByArea2());
    ...
    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public static px_picker.ui.camera.util.Size getPictureSize(px_picker.ui.camera.util.Size[] sizes, @CameraConfig.MediaQuality int mediaQuality) {
        if (sizes == null || sizes.length == 0) return null;

        List<px_picker.ui.camera.util.Size> choices = Arrays.asList(sizes);
    ...
    @SuppressWarnings("deprecation")
    public static px_picker.ui.camera.util.Size getOptimalPreviewSize(List<px_picker.ui.camera.util.Size> sizes, int width, int height) {
        final double ASPECT_TOLERANCE = 0.1;
        double targetRatio = (double) height / width;

        if (sizes == null) return null;

        px_picker.ui.camera.util.Size optimalSize = null;
        double minDiff = Double.MAX_VALUE;

        int targetHeight = height;

        for (px_picker.ui.camera.util.Size size : sizes) {
    ...
    @SuppressWarnings("deprecation")
    public static px_picker.ui.camera.util.Size getSizeWithClosestRatio(List<px_picker.ui.camera.util.Size> sizes, int width, int height) {
    ...
    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public static px_picker.ui.camera.util.Size getOptimalPreviewSize(px_picker.ui.camera.util.Size[] sizes, int width, int height) {
...
//                return size;
    ...
    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public static px_picker.ui.camera.util.Size getSizeWithClosestRatio(px_picker.ui.camera.util.Size[] sizes, int width, int height) {
...
//                return size;
    ...
    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public static px_picker.ui.camera.util.Size chooseOptimalSize(px_picker.ui.camera.util.Size[] choices, int width, int height, px_picker.ui.camera.util.Size aspectRatio) {
        // Collect the supported resolutions that are at least as big as the preview Surface
        List<px_picker.ui.camera.util.Size> bigEnough = new ArrayList<>();
        int w = aspectRatio.getWidth();
        int h = aspectRatio.getHeight();
        for (px_picker.ui.camera.util.Size option : choices) {
    ...
    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    private static class CompareSizesByArea2 implements Comparator<px_picker.ui.camera.util.Size> {
        @Override
        public int compare(px_picker.ui.camera.util.Size lhs, Size rhs) {
