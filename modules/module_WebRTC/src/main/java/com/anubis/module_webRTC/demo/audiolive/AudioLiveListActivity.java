...
public class AudioLiveListActivity extends BaseActivity implements AdapterView.OnItemClickListener, SwipeRefreshLayout.OnRefreshListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(com.anubis.module_webRTC.R.layout.activity_audio_live_list);
        ((TextView)findViewById(com.anubis.module_webRTC.R.id.title_text)).setText("互动语音直播列表");
                ...
                if(success){
                    ...
                    for(int i = 0;i<datas.length();i++){
                        try {
                            ...
                            AudioLiveInfo item = new AudioLiveInfo();
    ...
    private void queryAllList(){
        if(MLOC.INSTANCE.getAEventCenterEnable()){
            InterfaceUrls.demoQueryList(MLOC.INSTANCE.getLIST_TYPE_AUDIO_LIVE_ALL());
        }else{
            XHClient.getInstance().getLiveManager().queryList("", MLOC.INSTANCE.getLIST_TYPE_AUDIO_LIVE_ALL(),new IXHResultCallback() {
                @Override
...
//                    JSONArray array = (JSONArray) data;
    ...
    class MyListAdapter extends BaseAdapter{
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            viewIconImg.vHeadImage.setImageResource(com.anubis.module_webRTC.R.drawable.icon_main_mic);
            return convertView;
        }
